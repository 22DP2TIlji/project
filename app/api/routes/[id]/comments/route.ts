...
export async function GET(
  ...
  try {
    ...
    const comments = await (prisma as unknown as { routeComment: { findMany: (args: object) => Promise<Array<{ id: number; userId: number; text: string; createdAt: Date; user: { name: string } }>> } }).routeComment.findMany({
      where: { routeId },
...
export async function POST(
  ...
  try {
    ...
    const comment = await (prisma as unknown as { routeComment: { create: (args: object) => Promise<{ id: number; userId: number; text: string; user: { name: string }; createdAt: Date }> } }).routeComment.create({
